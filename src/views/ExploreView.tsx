  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-8">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 pb-6 border-b border-white/10">
        <div>
          <h1 className="font-heading text-3xl font-extrabold text-white">Explore Events Worldwide</h1>
          <p className="text-xs text-slate-400 mt-1 font-mono">Discover hybrid tech conferences, hackathons, concerts & VIP summits.</p>
        </div>

        {/* View Mode Toggle Switcher */}
        <div className="flex items-center gap-1 bg-white/5 border border-white/10 p-1 rounded-2xl self-start md:self-auto">
          <button
      ...
      <div className="p-4 bg-[#14161d] border border-white/10 rounded-2xl flex flex-col md:flex-row items-center gap-4">
        ...
        <div className="relative flex-1 w-full">
          ...
          <input
            ...
            placeholder="Search by city, title, topic or category..."
      ...
      {viewMode === 'map' && (
        <div className="relative h-[500px] bg-slate-950 border border-white/10 rounded-3xl overflow-hidden flex items-center justify-center p-6 text-center">
          ...
          <div className="relative z-10 space-y-4 max-w-md">
            ...
            <h3 className="font-heading text-xl font-bold text-white">Interactive World Map Active</h3>
            <p className="text-xs text-slate-400">
              Pin clusters loaded for San Francisco, New York, and London hubs.
            </p>
      ...
      {viewMode === 'calendar' && (
        <div className="p-6 bg-[#14161d] border border-white/10 rounded-3xl space-y-4">
          <h3 className="font-heading text-lg font-bold text-white">Event Calendar Stream</h3>
          <div className="space-y-3">
            {filteredEvents.map(e => (
              ...
              >
                <div>
                  <span className="text-xs font-mono text-[#00E5A8]">{e.date} • {e.time}</span>
                  <h4 className="font-heading text-base font-bold text-white mt-0.5">{e.title}</h4>
                  <p className="text-xs text-slate-400">{e.location.venueName}, {e.location.city}</p>
                </div>
